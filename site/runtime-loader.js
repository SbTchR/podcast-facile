...
const NEW_HISTORY_SOUNDS = [
  ...
  },
  {
    ...
    description: 'Crépitements, détonations et réactions du public. Peut évoquer une fête, une commémoration ou une tension lointaine, sans être présenté comme une bataille réelle.',
...
const CATEGORY_RENAMES = new Map([
  ['Histoire & action', 'Guerres & combats'], ['Lieux & ambiances', 'Sociétés & lieux historiques'],
  ['Nature & animaux', 'Nature & paysages'], ['Transports & machines', 'Transports & industrie'], ['Voix & personnes', 'Voix & foule']
]);
...
function setCategoryForSound(source, id, category) {
  ...
  if (objectStart === -1) { console.warn(`[Podcast Facile] Son introuvable pour reclassement : ${id}`); return source; }
  ...
  if (categoryStart === -1 || categoryStart > objectEnd) { console.warn(`[Podcast Facile] Catégorie introuvable pour : ${id}`); return source; }
  const valueStart = categoryStart + categoryMarker.length;
  const valueEnd = source.indexOf('`', valueStart);
  if (valueEnd === -1 || valueEnd > objectEnd) { console.warn(`[Podcast Facile] Catégorie illisible pour : ${id}`); return source; }
